...
//    protected double waferGraphRatio;
    ...
    MetrologyMap_WeightedObserverved_BufferedImage_XYShapeRenderer_NoColorRangeClass(String title, String fileTitle
        ...
        this.yMin = calcMin(this.XYCircleZ.get("Y"));
        this.yMax = calcMax(this.XYCircleZ.get("Y"));
        //
        this.zMin = calcMin(this.XYCircleZ.get("Z"));
        this.zMax = calcMax(this.XYCircleZ.get("Z"));
        //
        ...
        while (zScaleVal < this.zMax) {
            if ((cR > 0) && (!cRInc) && (!cGInc) && (cB == 255)) {
                if ((cR == 128) && (cG == 0) && (cB == 255)) {
                    List<Double> TempzScaleValues = new ArrayList<>();
                    //List<Integer> TempzScaleValues = new ArrayList<>();
                    //this.colorRanges.add(new ColorRange(0, (int) this.zMin, new Color(cR, cG, cB)));
                    TempzScaleValues.add(0D);
                    //TempzScaleValues.add((int) 0);
                    TempzScaleValues.add(zMin);
//                    TempzScaleValues.add(((int) zMin) *1D);
                    //TempzScaleValues.add((int) zMin);
                    colorRanges.put(new Color(cR,cG,cB), TempzScaleValues);
                } else {
                    Color tempColor = new Color(cR, cG, cB);
//                    this.colorRanges.add(new ColorRange((int) zScaleVal, (int) Math.min(zScaleVal + zIncrement, zMax)
//                            , new Color(cR, cG, cB)));
//                    double finalZScaleVal = ((int) zScaleVal) * 1D;
                    double finalZScaleVal = zScaleVal;
//                    int finalZScaleVal = (int) zScaleVal;
                    if (colorRanges.containsKey(tempColor)) {
                        colorRanges.computeIfPresent(tempColor, (k, v) -> {
                            v.add(finalZScaleVal);
                            return v;
                        });
                        colorRanges.computeIfPresent(tempColor, (k, v) -> {
                            //v.add(((int) Math.min(finalZScaleVal + zIncrement, zMax)) * 1D);
                            v.add(Math.min(finalZScaleVal + zIncrement, zMax));
//                            v.add((int) Math.min(finalZScaleVal + zIncrement, zMax));
                            return v;
                        });
                    } else {
                        colorRanges.computeIfAbsent(tempColor, tC -> new ArrayList<>()).add(zScaleVal);
//                        colorRanges.computeIfAbsent(tempColor, tC -> new ArrayList<>()).add((int) zScaleVal);
                        colorRanges.computeIfPresent(tempColor, (k, v) -> {
                            //v.add(((int) Math.min(finalZScaleVal + zIncrement, zMax)) * 1D);
                            v.add(Math.min(finalZScaleVal + zIncrement, zMax));
//                            v.add((int) Math.min(finalZScaleVal + zIncrement, zMax));
...
//                this.colorRanges.add(new ColorRange((int) zScaleVal, (int) Math.min(zScaleVal + zIncrement, zMax)
//                        , new Color(cR, cG, cB)));
                //colorRanges.put(zScaleVal, new Color(cR, cG, cB));
                Color tempColor = new Color(cR, cG, cB);
//                    double finalZScaleVal = ((int) zScaleVal) * 1D;
                    double finalZScaleVal = zScaleVal;
//                int finalZScaleVal = (int) zScaleVal;
...
//                        , new Color(cR, cG, cB)));
        ...
        // Add Max
        //colorRanges.put(zMax, new Color(cR, cG, cB));
        Color tempColor = new Color(cR, cG, cB);
        colorRanges.computeIfPresent(tempColor, (k, v) -> {
            //v.add(((int) zMax) * 1D);
            v.add(zMax);
...
//            v.add((int) Double.POSITIVE_INFINITY);
            return v;
        });
//        this.colorRanges.add(new ColorRange((int) this.zMax, (int) Double.POSITIVE_INFINITY
//                , new Color(255, 0, 0)));
        this.waferPaintScale.add(zMax, new Color(cR, cG, cB));
    }
    //
...
//        }
    ...
    private int getColor(double val) {
        ...
        //for (int c = 0; c < colorRanges.
//        if (val <= zMin) {
//            System.out.println("what is the val?: " + val);
////            System.out.println("what is the colorRanges?:\n" + colorRanges);
//            System.out.println("what is the colorRanges.entrySet()?:\n" + colorRanges.entrySet());
//            for (Map.Entry<Color, List<Double>> c : colorRanges.entrySet()) {
//                int colorListSize = c.getValue().size() - 1;
//                Double colorMin = c.getValue().get(0);
////                System.out.println("what is the colorMin?\n" + colorMin);
////                System.out.println("what is the c.getValue()?\n" + c.getValue());
//                Double colorMax = c.getValue().get(colorListSize);
//                if (val >= colorMin && val <= colorMax) {
//                    System.out.println("what is the colorMin?\n" + colorMin);
////                    System.out.println("what is the c.getValue()?\n" + c.getValue());
//                    return c.getKey().getRGB();
//                }
//            }
//        }
        for (Map.Entry<Color, List<Double>> c : colorRanges.entrySet()) {
//        for (Map.Entry<Color, List<Integer>> c : colorRanges.entrySet()) {
            int colorListSize = c.getValue().size() - 1;
            Double colorMin = c.getValue().get(0);
//            int colorMin = c.getValue().get(0);
            Double colorMax = c.getValue().get(colorListSize);
//            int colorMax = c.getValue().get(colorListSize);
            if (val >= colorMin && val < colorMax) {
                return c.getKey().getRGB();
            }
        }
//        return colorRanges.getPaint(val);
//        for (LookupPaintScale r : colorRanges) {
